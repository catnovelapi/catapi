...
func (cat *Ciweimao) ChaptersCatalogApi(bookId string) (gjson.Result, error) {
	return cat.Req.Post(catalogApiPoint, map[string]string{"book_id": bookId})
}

func (cat *Ciweimao) ChaptersCatalogV2Api(bookId string) (gjson.Result, error) {
	if catalog, err := cat.Req.Post(catalogNewApiPoint, map[string]string{"book_id": bookId}); err != nil {
		return gjson.Result{}, err
	} else {
...
func (cat *Ciweimao) ReviewListApi(bookId string, page string) (gjson.Result, error) {
	if result, err := cat.Req.Post(reviewListApiPoint, map[string]string{"book_id": bookId, "count": "10", "page": page, "type": "1"}); err != nil {
		return gjson.Result{}, err
...
func (cat *Ciweimao) ReviewCommentListApi(reviewId string, page string) (gjson.Result, error) {
	if result, err := cat.Req.Post(bookReviewCommentListApiPoint, map[string]string{"review_id": reviewId, "count": "10", "page": page}); err != nil {
		return gjson.Result{}, err
	} else {
...
func (cat *Ciweimao) ReviewCommentReplyListApi(commentId string, page string) (gjson.Result, error) {
	if result, err := cat.Req.Post(reviewCommentReplyListApiPoint, map[string]string{"comment_id": commentId, "count": "10", "page": page}); err != nil {
		return gjson.Result{}, err
...
func (cat *Ciweimao) SignupApi(account string, password string) (gjson.Result, error) {
	return cat.Req.Post(loginApiPoint, map[string]string{"login_name": account, "passwd": password})
}
func (cat *Ciweimao) ChapterCommandApi(chapterId string) (string, error) {
	if commandInfo, err := cat.Req.Post(chapterCommandApiPoint, map[string]string{"chapter_id": chapterId}); err != nil {
...
func (cat *Ciweimao) TsukkomiNumApi(chapterID string) (gjson.Result, error) {
	return cat.Req.Post(chapterTsukkomiNumApiPoint, map[string]string{"chapter_id": chapterID})
}
...
func (cat *Ciweimao) contentInfoApi(chapterId string) (gjson.Result, string, error) {
	...
	chapterInfo, err := cat.Req.Post(chapterInfoApiPoint, map[string]string{"chapter_id": chapterId, "chapter_command": command})
	if err != nil {
		return gjson.Result{}, "", err
	}
...
func (cat *Ciweimao) UseGeetestInfoApi(loginName string) (int, error) {
	useGeetest, err := cat.Req.Post(useGeetestApiPoint, map[string]string{"login_name": loginName})
	if err != nil {
		return 0, err
	}
...
func (cat *Ciweimao) BookmarkListApi(bookID string, page string) (gjson.Result, error) {
	return cat.Req.Post("/book/get_bookmark_list", map[string]string{"count": "10", "book_id": bookID, "page": page})
}
func (cat *Ciweimao) BdaudioInfoApi(bookID string) (gjson.Result, error) {
	return cat.Req.Post("/reader/bdaudio_info", map[string]string{"book_id": bookID})
}

func (cat *Ciweimao) AddReadbookApi(bookID string, readTimes string, getTime string) (gjson.Result, error) {
	return cat.Req.Post("/reader/add_readbook", map[string]string{"book_id": bookID, "readTimes": readTimes, "getTime": getTime})
}

func (cat *Ciweimao) SetLastReadChapterApi(lastReadChapterID string, bookID string) (gjson.Result, error) {
	return cat.Req.Post("/bookshelf/set_last_read_chapter", map[string]string{"last_read_chapter_id": lastReadChapterID, "book_id": bookID})
}
func (cat *Ciweimao) PostPrivacyPolicyVersionApi() (gjson.Result, error) {
	return cat.Req.Post("/setting/privacy_policy_version", map[string]string{"privacy_policy_version": "1"})
}
